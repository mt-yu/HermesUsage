...
为什么存在
----------
sources/citations.yaml 里 95 条 URL 全部指向 hermes-agent.nousresearch.com。
...
怎么跑
------
  python scripts/check_links_external.py                    # 默认全量 95 条
...
def opener() -> urllib.request.OpenerDirector:
    """显式清空代理：本机默认代理已挂，否则 95 条全会变成 network。"""
...
def check_one(entry: dict, timeout: float = 15.0, retries: int = 2, backoff: float = 1.5) -> dict:
    ...
    不重试的话，一次抖动就会把 95 条全报成 network —— 看起来像"官方文档全挂"，
