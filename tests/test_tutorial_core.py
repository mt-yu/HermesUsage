...
class TestLoadLessonsOnRealRepo(unittest.TestCase):
    ...
    def test_count_matches_repo(self):
        self.assertEqual(len(self.lessons), 39)
...
class TestPitfallRows(unittest.TestCase):
    ...
    页面上的 324 行全部来自这里，解析口径一变，页面上就会多出表头、
    ...
    def test_repo_total_is_324_rows(self):
        # 阶段 6 六课加入后从 253 涨到 312；这个字面量是金丝雀：内容大批增减时要有人来看一眼
        self.assertEqual(sum(len(rows) for rows in self.repo_rows.values()), 324)
...
class TestGrouping(unittest.TestCase):
    def test_groups_cover_all_stages(self):
        ...
        self.assertEqual(sum(len(g["lessons"]) for g in groups), 39)
