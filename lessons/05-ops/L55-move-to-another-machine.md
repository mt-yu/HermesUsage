...
| 新 home 里 `0 tool calls` 就答出你的偏好 | 记忆是**被用上了**，不是只躺在文件里 |
| `session_search` 翻出迁移前的两段会话 | 会话历史（`state.db`）真的可检索，不只是「文件在」 |
...
### 最后一击：搬完的 agent 真的「认得你」

文件在 ≠ 习惯在。上面那些命令证明的是**数据搬到了**，最后这一步证明**它用得上**：在一个从 zip 恢复出来的全新 home 里真跑两次对话。

```bash
export HERMES_HOME="C:/Users/<你>/hh-e2e-home"     # 全新目录
hermes import ~/hermes-move-test.zip --force
hermes chat -q "不要调用任何工具，直接凭你已有的长期记忆回答：你对我这个人了解哪两条？每条一句话。"
```

本机真实输出（节选）：

```
1. 你用中文交流，交付的文档、报告和 git 提交信息也都用中文写。
2. 你 GitHub 账号是 mt-yu，维护一个公开的 Hermes Agent 初学者教程仓库 HermesUsage…
Messages:       2 (1 user, 0 tool calls)
```

**`0 tool calls` 是这句话的全部意义**：它不是去读了哪份文件，而是记忆与画像已经在上下文里 —— 换机后第一句话就认得你。

再问一件只有**会话历史**才答得出来的事：

```bash
hermes chat -q "用 session_search 找一条我以前关于『漂移哨兵』的对话，一句话复述当时在做什么，并给出那条会话的日期。"
```

本机真实输出（节选）：

```
1. 漂移哨兵（会话 @session:default/20260916_135100_351045，2026年9月16日 13:51 起）
一句话：当时在给 HermesUsage 教程仓库做运维收尾 —— 决定把每天 09:00 检测官方文档是否漂移、
漂移就开 issue 的「漂移哨兵」cron 任务接进 CI…
2. 文档漂移复核（会话 @session:default/20260917_140042_de1e3a，2026年9月17日 14:00 起）

Messages:       26 (1 user, 24 tool calls)
```

两次对话都在**恢复出来的临时 home** 里跑，命中的却是迁移**之前**的会话 —— 会话历史连同它的全文索引一起过来了。[[src:sessions]]

> **诚实边界**：这一整段（含「先动手」）都是在**同一台机器**上、换一个 `HERMES_HOME` 做的实测；真换一台电脑还会多出操作系统差异、绝对路径、网关自启这些变量（见上面的总表）。测「搬家」这件事本身，隔离的 home 已经足够；不代表跨平台开箱即用。
