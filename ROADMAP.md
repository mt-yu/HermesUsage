...
**为什么不为这些未发布功能单独开一节「下个版本预告」课**（2026-09-21 决定）：R3 要求每句事实性
陈述都能反查到**读者装得到的版本**的官方文档；一节讲「你还没有的功能」的课，读者照着做必然撞墙，
而且每次上游改动都要重写。这份清单已经有两个更好的载体：① 上表（人读）；
② `scripts/pending_probe.py`（机器读，一条命令回答「落地了吗」）。**功能真正发布那天**，
按清单把它写进它本该在的那一课 —— 那时它才是可考证的。
...
现状（2026-09-24 实测，v3.6.1 回填后）：内容 **43 课**、出处 **97**、`check.py` 七项全绿（Python 445 条 + 前端 75 条）、站点自检 43 课 / 55 页 / 73 文件 / 4285 KB。
